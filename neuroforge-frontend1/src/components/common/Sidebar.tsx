...
export default function Sidebar() {
  ...
  // `role` is derived from the authenticated user's real backend role (see
  // AuthContext / roleUtils) — the same single source of truth the top
  // profile menu uses, so these can never disagree.
  const effectiveRole = role;

  const displayRole = effectiveRole ? getRoleDisplayName(effectiveRole) : 'User';
    ...
    switch (effectiveRole) {
      ...
      case 'org-admin':
        return [
          ...
          { label: 'Portfolio',       icon: BarChart3,       href: '/org-admin/portfolio' },
      ...
      case 'project-manager':
        return [
          ...
          { label: 'Portfolio',       icon: BarChart3,       href: '/project-manager/portfolio' },
          { label: 'Projects',        icon: FolderKanban,    href: '/project-manager/projects' },
          { label: 'Sprints',         icon: GitBranch,       href: '/project-manager/sprints' },
      ...
      case 'developer':
        return [
          ...
          { label: 'Projects',        icon: FolderKanban,    href: '/developer/projects' },
