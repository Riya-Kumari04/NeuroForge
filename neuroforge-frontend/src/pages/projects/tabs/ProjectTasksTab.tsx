...
function TaskModal({ task, projectId, sprintOptions, memberOptions, onClose, readOnlyStatus }: {
  ...
  onClose: () => void;
  readOnlyStatus?: boolean; // developer/tester: only status can be changed
}) {
...
export default function ProjectTasksTab({ project }: Props) {
  ...
  const canUpdate = canUpdateTasks(role);   // Developer/Tester can update status
    ...
    <div className="space-y-5">
                  ...
                  {!isClient && (
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-2">
                        {/* Edit — developer/tester can update status; PM+ full edit */}
                        {canUpdate && (
                          <button
      ...
      {modal !== null && (
        <TaskModal
          ...
          readOnlyStatus={!canWrite && canUpdate} // developer/tester: status-only
        />
      )}
      {canWrite && (
        <ConfirmDialog
